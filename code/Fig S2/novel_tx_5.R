...
file1 <- fread('./data/target10.bedgraph', col.names = c('chromosome', 'start', 'end', 'value'))
file2 <- fread('./data/target10.rev.TSS.txt', col.names = c('chromosome', 'start', 'end', 'value'))
file3 <- fread('./data/target10.rev.CPAS.txt', col.names = c('chromosome', 'start', 'end', 'value'))

file1<-file1[file1$start>myStart]
file1<-file1[file1$end<myEnd,]
max1<-max(file1$value)

dataTrack1 <- DataTrack(range = file1, type = "a", chromosome=myChr, genome = 'HG38', fill = "goldenrod1", col = "goldenrod1", options(ucscChromosomeNames=FALSE),col.axis="black", background.title = "transparent", ylim=c(0,100))
...
dataTrack3 <- DataTrack(range = file3, type = "a", chromosome=myChr, genome = 'HG38', fill = "black", col = "black", options(ucscChromosomeNames=FALSE),col.axis="black", background.title = "transparent", ylim=c(0,100))
...
Regions<-read.table("data/novel_tx_5.regions.txt", header=T, sep="\t")
