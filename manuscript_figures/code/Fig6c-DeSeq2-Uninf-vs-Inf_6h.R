...
### Read in count files
UNINF_1 <- read.csv('./Uninf-1.merged.counts.txt', sep=',', header = TRUE)
UNINF_2 <- read.csv('./Uninf-2.merged.counts.txt', sep=',', header = TRUE)
INF_1 <- read.csv('./HSV1-6hpi-1.merged.counts.txt', sep=',', header = TRUE)
INF_2 <- read.csv('./HSV1-6hpi-2.merged.counts.txt', sep=',', header = TRUE)

### drop first and last column
UNINF_1 <- UNINF_1[, -c(1, ncol(UNINF_1))]
UNINF_2 <- UNINF_2[, -c(1, ncol(UNINF_2))]
INF_1 <- INF_1[, -c(1, ncol(INF_1))]
INF_2 <- INF_2[, -c(1, ncol(INF_2))]
...
write.csv(result, "deseq2-INF-vs-UNINF.output.csv", row.names=TRUE)
...
# Step 4: Subset into Pol III and Pol II
...
pdf(file = "volcano-INF6h-vs-UNINF.pdf", width = 7, height = 9)
print(volc)
dev.off()
