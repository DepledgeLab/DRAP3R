...
filepaths <- all_files[grepl("pseU", basename(all_files), ignore.case =TRUE)] ### Change pseU -> m6A for m6A plotting
filepaths
df_list <- list()
##-------------FILE SELECTION---------------------##
...
merged_df <- merged_df %>% mutate(DataBase = case_when(
  ...
  grepl("EBER2-IVT", DataBase, ignore.case = TRUE) & grepl("noFilt", DataBase, ignore.case = TRUE) ~ "EBER2-IVT-3 - Unfiltered",
