...
COV <- 20
FREQ <- 1 #10 use 10 for scatterplots / stoichiometry analysis
...
file_groups <- list(
  pseU_sites = list(
    files = list(
      "ARPE19_HSV1_6hpi-1.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.hg38-tRNAs.intersect.bed",
      "ARPE19_HSV1_6hpi-2.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.hg38-tRNAs.intersect.bed",
      "ARPE19-HSV1-12hpi-1.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.hg38-tRNAs.intersect.bed",
      "ARPE19-HSV1-12hpi-2.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.hg38-tRNAs.intersect.bed",
      ...
      "ARPE19-IFNag-1.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.hg38-tRNAs.intersect.bed"
    ),
    variable_names = list(
      "HSV1_6hpi_1_tRNA_pseU_sites",
      "HSV1_6hpi_2_tRNA_pseU_sites",
      "HSV1_12hpi_1_tRNA_pseU_sites",
      "HSV1_12hpi_2_tRNA_pseU_sites",
 ...
 PolIIItx_pseU_sites = list(
    files = list(
      "ARPE19_HSV1_6hpi-1.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.PolIIItx.primaryOnly.intersect.bed",
      "ARPE19_HSV1_6hpi-2.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.PolIIItx.primaryOnly.intersect.bed",
      "ARPE19-HSV1-12hpi-1.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.PolIIItx.primaryOnly.intersect.bed",
      "ARPE19-HSV1-12hpi-2.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.PolIIItx.primaryOnly.intersect.bed",
      ...
      "ARPE19-IFNag-1.sup-pseU_bwa_W13k6T20.SR.primary-merged.HG38.modFilt.0.98.PolIIItx.primaryOnly.intersect.bed"
    ),
    variable_names = list(
      "HSV1_6hpi_1_PolIIItx_pseU_sites",
      "HSV1_6hpi_2_PolIIItx_pseU_sites",
      "HSV1_12hpi_1_PolIIItx_pseU_sites",
      "HSV1_12hpi_2_PolIIItx_pseU_sites",
...
h12_2_tRNA<-ggplot() +
  geom_histogram(data = HSV1_12hpi_2_tRNA_pseU_sites, aes(x = frac_mod), 
                 binwidth = 2, fill = "#5979bc", alpha = 0.9, color = "black", boundary = 0) +  # First histogram
  scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 20)) +
  labs(x = "Modification stoichiometry", y = "Count") + #, title = "ARPE19 rep2 pre-tRNA") +  # Axis labels and title
  theme_classic() +
...
h12_2_pIII<-ggplot() +
  geom_histogram(data = HSV1_12hpi_2_PolIIItx_pseU_sites, aes(x = frac_mod), 
                 binwidth = 2, fill = "#5979bc", alpha = 0.9, color = "black", boundary = 0) +  # First histogram
  scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 20)) +
  labs(x = "Modification stoichiometry", y = "Count") + #, title = "ARPE19 rep1 Pol III tx") +  # Axis labels and title
  theme_classic() +
...
combined_plot2 <- h6_1_tRNA + h6_2_tRNA + h12_1_tRNA + h12_2_tRNA + IFN_1_tRNA + h6_1_pIII + h6_2_pIII + h12_1_pIII + h12_2_pIII + IFN_1_pIII + plot_layout(ncol = 5)

combined_plot3 <- h6_1_tRNA + h6_1_pIII + h6_2_tRNA + h6_2_pIII + h12_1_tRNA + h12_1_pIII + h12_2_tRNA + h12_2_pIII + IFN_1_tRNA + IFN_1_pIII + plot_layout(ncol = 2)


pdf(file = "HSV1_FreqHistograms.pdf", width = 20, height = 10)
combined_plot2
dev.off()

pdf(file = "HSV1_FreqHistograms_Alt.pdf", width = 10, height = 20)
combined_plot3
dev.off()
...
inf2_comp <- merge(HSV1_12hpi_1_tRNA_pseU_sites, HSV1_12hpi_2_tRNA_pseU_sites, by = c("start", "end", "tRNA"), all = TRUE)
inf2_comp <- na.omit(inf2_comp)
inf2_comp <- inf2_comp[, c("start", "end", "tRNA", "frac_mod.x", "frac_mod.y")]
inf2_R <- cor(inf2_comp$frac_mod.x, inf2_comp$frac_mod.y, method="spearman")
inf2_R2 <- inf2_R^2

inf2<-ggplot(inf2_comp, aes(x = frac_mod.x, y = frac_mod.y)) +
  geom_point(color = "#5979bc", alpha = 0.5) +  # Scatter plot with blue points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed", size = 1) +  # x = y line (y = x)
  labs(x = "HSV-1 12 hpi #1", y = "HSV-1 12 hpi #2", 
       title = paste("Pearson r2=", round(inf2_R, 4))) + 
  theme_classic() +
  theme(plot.title = element_text(size = 20), 
        text = element_text(size = 22, colour = "black"),  # Set all text to black
        axis.line = element_line(size = 1, color = "black"),  # Thicken axis lines
...
comp6_long <- comp6 %>%
  select(frac_mod.x, frac_mod.y) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "value")

# Create the ggplot boxplot
box6<-ggplot(comp6_long, aes(x = variable, y = value, fill = variable)) +
  geom_boxplot() +
  scale_fill_manual(values = c("frac_mod.x" = "lightskyblue", "frac_mod.y" = "dodgerblue1")) +
  labs(
    title = "Distribution of frac_mod Values",
    y = "Value",
    x = ""
  ) +
  theme_classic() +
  theme(plot.title = element_text(size = 20), 
        text = element_text(size = 22, colour = "black"),  # Set all text to black
        axis.line = element_line(size = 1, color = "black"),  # Thicken axis lines
        axis.text = element_text(size = 20, colour = "black")) + theme(legend.position = "none") # Black axis text
...
### comparison of stoichiometries: uninf vs 12hpi
...
# Create the ggplot boxplot
box12<-ggplot(comp12_long, aes(x = variable, y = value, fill = variable)) +
  geom_boxplot() +
  scale_fill_manual(values = c("frac_mod.x" = "lightskyblue", "frac_mod.y" = "dodgerblue1")) +
  labs(
    title = "Distribution of frac_mod Values",
...
pdf(file = "tRNA_stoichiometry_changes_freq0.01.pdf", width = 10, height = 10)
combined_plot4
dev.off()

sensibles <- (inf | inf2) 

pdf(file = "selfcomp_tRNA_stoichiometry_changes_freq0.01.pdf", width = 10, height = 5)
sensibles
dev.off()

pdf(file = "IFN_tRNA_stoichiometry_changes_freq0.01.pdf", width = 5, height = 5)
IFN
dev.off()

combined_w_box <- ((c6 | box6) / (c12 | box12)) +
  plot_layout(widths = c(0.7, 0.3)) 

combined_w_box

pdf(file = "tRNA_stoichiometry_changes_w_boxplot_freq0.01.pdf", width = 10, height = 10)
combined_w_box
dev.off()

boxplots <- box6 / box12

pdf(file = "boxplot_freq0.01.pdf", width = 3, height = 10)
boxplots
dev.off()
