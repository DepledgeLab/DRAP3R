...
data <- read.csv("FigS1f-ARPE19_uninf-1_bwa_W13k6T20.merged.primary.sorted.HG38.bed", sep="\t", header=FALSE)
data<-data[, c("V2","V3","V4")]
data$aln_length <- data$V3 - data$V2
colnames(data) <- c("start","stop","read_id","aln_length")

SeqSum <- read.table("FigS1ef-ARPE19_uninf-1.notrim.seqSum.txt", sep="\t", header=F, stringsAsFactors=FALSE)
...
p<-ggplot(merged_sampled, aes(sequence_length, aln_length)) +
  ...
  labs(title = "bwa -W13 -k6 -T20 primary alignments", x = "read length", y = "alignment length") +
...
pdf(file = "readLengthVSalnLength_bwa_W13k6T20_primaryOnly.pdf", width = 7, height = 6)
p
dev.off()
